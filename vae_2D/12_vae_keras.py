...
class VAE(keras.Model):
    ...
    def train_step(self, data):
        with tf.GradientTape() as tape:
            ...
            print(f'kl_loss={kl_loss}, kl_loss shape={tf.shape(kl_loss)}')
            kl_loss = tf.reduce_mean(tf.reduce_sum(kl_loss, axis=1))

            # sparse experiments
            beta=10
            l1_loss=beta*tf.abs(z)
            l1_loss = tf.reduce_mean(tf.reduce_sum(l1_loss, axis=1))

            print(f'z={z}, z shape={tf.shape(z)}')
...
if __name__ == "__main__":
    ...
    vae.fit(x_train, epochs=10, batch_size=128, callbacks=my_callbacks)
    ...
    print(z[2])
    out=tf.abs(z[2])
    print(tf.reduce_mean(out,axis=0))
